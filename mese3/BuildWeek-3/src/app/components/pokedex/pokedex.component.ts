...
export class PokedexComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.subscriptions.push(
            this.pkSrv
                .getTeamMembers(this.user!.user.id)
                .subscribe((data) => (this.pokemonTeam = data))
        );
    }

    submit() {
        this.nomePokemon = this.form.value.name;
    ...
    type(type: string) {
        this.tipo = type;
        this.pkSrv.getPokemon().subscribe((res) => {
            this.pokemon = res.filter(
                (pokemon) =>
                    this.tipo == pokemon.type[0] || this.tipo == pokemon.type[1]
            );
        });
    }
